...
class CustomFTTransformerClassifier(CustomBaseModel):

    def __init__(self, **kwargs: Any) -> None:

        super().__init__()
        self.model: FTTransformer = FTTransformer(**kwargs)
        print("[INFO] FTTransformer model initialized.")
    ...
    def fit(
        ...
        if cat_columns is None or cont_columns is None:
            raise ValueError("cat_columns and cont_columns must be specified")
        ...
        print("[INFO] Model moved to device:", device)
        ...
        for epoch in range(MAX_EPOCH):
            ...
            print(
                f"[INFO] Epoch {epoch + 1}/{MAX_EPOCH} - Train Loss: {avg_train_loss:.4f}, Valid Loss: {avg_valid_loss:.4f}"
            )

            if epoch >= 2:
                last_three_train = train_losses[-3:]
                if max(last_three_train) - min(last_three_train) <= 0.001:
                    print("[INFO] Early stopping: Train loss change is less than 0.001 over 3 consecutive epochs.")
                    break

                last_three_valid = valid_losses[-3:]
                if all(v > t for v, t in zip(last_three_valid, train_losses[-3:])):
                    print("[INFO] Early stopping: Validation loss is higher than Train loss for 3 consecutive epochs.")
                    break
    ...
    def save(self, model_path: str) -> None:
        ...
        save_dict = {
            ...
            "classes": self.le.classes_ if hasattr(self, "le") and self.le is not None else None,
        }
        torch.save(save_dict, model_path)
        print(f"[INFO] Model saved to {model_path}")

    @classmethod
    def load(cls, model_path: str, **kwargs: Any) -> 'CustomFTTransformerClassifier':
